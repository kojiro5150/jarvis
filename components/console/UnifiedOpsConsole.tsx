...
type PendingHandoff = { sourceId: string; targetId: string; originalMessage: string };
type ConnectorName = "calendar" | "gmail" | "drive";
type ConnectorServiceStatus = "online" | "refresh_required" | "unavailable" | string;
...
export default function UnifiedOpsConsole() {
  ...
  const fetchOperationalState = async (live = true) => {
    try {
      ...
      if (live) setConnectorStatuses({ calendar: "unavailable", gmail: "unavailable", drive: "unavailable" });
    }
  };
  ...
  const connectedCount = connectorStatuses ? Object.values(connectorStatuses).filter(status => status === "online").length : 0;
  ...
  const googleConnectAction = (status: ConnectorServiceStatus): { label: string; title: string; navigate: boolean } => {
    if (status === "online") {
      return { label: "DISCONNECT", title: "Disconnect Google — this also disconnects Calendar, Gmail, and Drive, since all three share one account grant.", navigate: false };
    }
    return {
      label: status === "refresh_required" ? "RECONNECT" : "CONNECT",
      title: status === "refresh_required" ? "Reconnect Google — the stored token no longer works for this service." : "Connect Google — grants Calendar, Gmail, and Drive together in one consent screen.",
      navigate: true,
    };
  };
  ...
  const renderConnectorStatus = (name: ConnectorName) => {
    ...
    return <button className="connector-action" type="button" title={action.title} onClick={() => void handleGoogleAction(connectorStatuses[name])}>{action.label}</button>;
  };
  ...
  async function send(event: FormEvent) {
    ...
    if (target) {
      setPendingHandoff({ sourceId: selected.id, targetId: target.id, originalMessage });
    }
  }
  ...
  return (
    <main className="console-page">
      <section className="console-frame">
        <button className={`sidebar-backdrop ${sidebarOpen ? "open" : ""}`} type="button" aria-label="Close specialist navigation" onClick={() => setSidebarOpen(false)} />
        <aside className={`sidebar ${sidebarOpen ? "open" : ""}`} aria-label="Specialist navigation">
          ...
          <button className={`executive ${selectedId === jarvis?.id ? "selected" : ""}`} type="button" disabled={!jarvis} onClick={() => jarvis && selectSpecialist(jarvis.id)}><span className="jarvis-icon">◎</span><span className="specialist-copy"><b>JARVIS</b><small>Orchestrator</small></span>{jarvis && selectedId === jarvis.id && (loading || (conversations[jarvis.id]?.length ?? 0) > 0) ? <span className="active">● ACTIVE</span> : <span className="ready">● READY</span>}</button>
          {dawnwatch ? <button className={`dawnwatch ${selectedId === dawnwatch.id ? "selected" : ""}`} type="button" onClick={() => selectSpecialist(dawnwatch.id)}><span className="dawn-icon">☀</span><span className="specialist-copy"><b>DAWNWATCH</b><small>{dawnwatch.purpose}</small></span>{selectedId === dawnwatch.id && (loading || (conversations[dawnwatch.id]?.length ?? 0) > 0) ? <span className="active">● ACTIVE</span> : <span className="ready">● READY</span>}</button> : <div className="dawnwatch loading-tile">Loading DAWNWATCH…</div>}
        ...
        <div className="command-area">
          ...
          <div className="workspace">
            ...
            <div className="conversation-frame"><div className="spin-border" /><div className="conversation">
              ...
              <div className="tools"><button type="button" disabled>🎙<small>Voice</small></button><button className="brief" type="button" disabled={!jarvis || loading} onClick={() => void briefMe()}>▤<small>Brief Me</small></button><button type="button" disabled={!jarvis} onClick={() => jarvis && selectSpecialist(jarvis.id)}>⬡<small>Ask JARVIS</small></button></div>
            </div></div>
          </div>
