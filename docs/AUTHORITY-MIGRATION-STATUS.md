...
| `PendingAuthorization` confirmation | △ | An isolated public resolver binds deterministic confirmation to one exact pending operation and scope, consumes it on `ALLOW`, and gives bare confirmation no authority. Production conversation state is not yet integrated. |
