...
deterministic release envelopes and prior exact `gmail.read` authority commands are replaced in a
fresh model-only history with fixed omission markers. This prevents both released fields and the
message IDs embedded in prior read commands from entering ordinary context. The current raw user
utterance remains intact through deterministic authority recognition, and the client-visible
transcript is neither mutated nor rewritten.

The boundary derives no trust from client-carried authority, provenance, or other metadata and
does not establish or claim durable provenance. A
