...
> **USER-CONTINUITY-CAPTURE-07:** For the first explicit capture experiment, capture intent is recognised only by a small deterministic, hand-maintained grammar for unmistakable current-turn remember/retain instructions. A model may not decide whether the user authorised durable capture. Inputs outside that closed grammar remain ordinary conversation unless and until a separately governed adaptive-intent boundary is designed and verified. The grammar is a bounded command surface, not a general natural-language coverage strategy.

> **USER-CONTINUITY-CAPTURE-08:** Semantic class must not be guessed before an append-only write. If the closed classifier cannot establish exactly one admissible class from `user_assertion | preference | plan | commitment | decision`, the capture attempt must fail closed before persistence and request clarification. Any model-assisted classifier must expose an explicit non-committal/ambiguous outcome in its closed contract; ambiguous output cannot be coerced to the nearest class. A later clarification may authorise the class for the same user-supplied content, but it may not silently rewrite that content or turn the model's rationale into user authorship.
