...
Sprint 3.161 audited the missing Calendar membership-completeness seam; Sprint 3.162 governed it; Sprint 3.163 implemented it in production; Sprint 3.164 selected **removed** as the only membership change currently ready for attention semantics; Sprint 3.165 implemented that removal policy as an isolated deterministic adapter; Sprint 3.166 confirmed the existing `calendar_attention_brief` is the correct publication artefact; Sprint 3.167 widened that brief to the closed start-time-or-removal policy-match union. Sprint 3.168 now implements deterministic rendering for removal, mixed-policy and zero-match cases directly — no additional readiness audit was needed because the remaining rendering questions were already bounded by 3.166 and the publication prerequisite was completed by 3.167. The next step is Sprint 3.169 — wire the already-governed removal selector into the existing live `What needs my attention?` path.
