...
Identity, roles, projects, commitments, communications, waiting items, explicit source- or user-assigned priorities, active work and current context are represented as readonly plain data. OperationalCommunication is governed by its [Constitutional Responsibility Statement](./OPERATIONAL-COMMUNICATION-RESPONSIBILITY.md): it records only intrinsic communication observations and protocol-asserted reply references, independently from OperationalCommitment and without thread reconstruction or semantic inference. Explicit priority is a supplied operational fact, not future Attention Engine ranking. Dates and instants are supplied ISO 8601 / RFC 3339 strings; the model does not read time or calculate lateness, urgency, conflicts or staleness.
