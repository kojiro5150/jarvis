...
describe("canonical runtime state and descriptive context prefix", () => {
  it("publishes one continuous immutable state/context lineage on deterministic replay", () => {
    ...
    for (const stage of first.trace.stages.slice(3, -6)) {
    ...
    expect(first.trace.stages.slice(-6)).toEqual([
      ...
      expect.objectContaining({ stageId: "executive_run_record", outputArtifactIds: [first.executiveRunRecord.executiveRunRecordId] }),
    ]);
    ...
    expect(first.executiveCapabilityInvocationHandoff.routingPlanId).toBe(first.capabilityRoutingPlan.routingPlanId);
    expect(first.capabilityInvocationEnvelope.handoffId).toBe(first.executiveCapabilityInvocationHandoff.handoffId);
