...
import { DeterministicExecutiveDeliberationContextEngine } from "../deliberation";
...
export class DeterministicExecutiveOperatingSystemRuntime {
 ...
 run(raw:ExecutiveOperatingSystemInput):ExecutiveOperatingSystemResult {
  ...
  const executiveDeliberationContext=step("executive_deliberation_context",[...canonicalIds,changes.previousSnapshotId,changes.currentSnapshotId,assessment.situationSetId],()=>new DeterministicExecutiveDeliberationContextEngine().construct({executiveContextSnapshot,assessmentSet:assessment,lifecycleComparison:changes,configuration:input.configuration.deliberation}),x=>[x.deliberationContextId!]);
  const context=executiveDeliberationContext;
  const intentAndConstraints=step("intent_and_constraints",[...canonicalIds,context.deliberationContextId!],()=>({intent:new DeterministicIntentEngine(new ExecutiveIntentRegistry(INITIAL_INTENT_POLICIES)).construct(context,input.configuration.intent),constraints:new DeterministicConstraintEngine(new ExecutiveConstraintRegistry(INITIAL_CONSTRAINT_POLICIES)).construct(context,input.configuration.constraints)}),x=>[x.intent.intentSetId,x.constraints.constraintSetId],x=>x.intent.objectives.length===0&&x.constraints.constraints.length===0);
  const {intent,constraints}=intentAndConstraints;
  const candidatePlans=step("candidate_plan_construction",[...canonicalIds,context.deliberationContextId!,intent.intentSetId,constraints.constraintSetId],()=>new DeterministicCandidatePlanConstructionEngine(new DeterministicCandidatePlanRegistry(productionCandidatePlanPolicies)).construct({context,intentSet:intent,constraintSet:constraints,definitions:input.configuration.candidatePlanDefinitions}),x=>[x.candidatePlanSetId],x=>x.candidates.length===0);
