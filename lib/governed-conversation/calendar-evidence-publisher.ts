...
export interface GovernedCalendarEvidenceSet {
  ...
  readonly coverageState: GovernedCalendarPublicationInput["coverageState"];
  readonly coverageLimit: string;
  readonly policyReference: typeof CALENDAR_CONVERSATIONAL_DISCLOSURE_POLICY;
...
export function publishCalendarEvidenceSet(input: GovernedCalendarPublicationInput): GovernedCalendarEvidenceSet | null {
  if (input.sourceId !== "google-calendar" || input.availability !== "available" || !timestamp(input.retrievedAt) ||
      !nonempty(input.windowStart) || !nonempty(input.windowEnd) || !Number.isInteger(input.requestedLimit) || input.requestedLimit < 0) return null;
  ...
  const evidence = Object.freeze(input.events.flatMap(event => {
...
export function publishCalendarEvidence(input: GovernedCalendarPublicationInput): readonly GovernedCalendarEvidenceInput[] {
  return publishCalendarEvidenceSet(input)?.evidence ?? Object.freeze([]);
}
