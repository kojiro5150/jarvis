...
describe("pure-Node isolation proof", () => {
  ...
  it("finds no hidden production import elsewhere", () => { const candidates = [...files("app"), ...files("lib")].filter(x => /\.(ts|tsx)$/.test(x) && !x.includes("claim-boundary") && !x.includes("conflict-boundary") && !x.includes("claims-conflicts-correction") && !x.includes("governed-publication-test-fixtures") && !x.includes("projection-composer.ts") && !x.includes("conflict-boundary-publications.ts")); for (const path of candidates) for (const name of boundaryNames) expect(readFileSync(path, "utf8"), path).not.toContain(name); });
});
