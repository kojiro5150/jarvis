...
type Draft = { readonly base: GovernedClaimInput; readonly status: GovernedClaimInput["status"]; readonly ownership: GovernedClaimInput["ownership"]; readonly sourceReferences: readonly GovernedSourceReference[]; readonly factualValues: readonly unknown[]; readonly sourceAvailable: boolean; readonly provenance: string; readonly observedAt: string; readonly boundedComplete: boolean; readonly outcome: ClaimEnrichmentRecord["outcome"]; readonly admittedCategories: ClaimEnrichmentRecord["admittedEvidenceCategories"]; readonly consulted: readonly GovernedSourceReference[]; readonly admitted: readonly GovernedSourceReference[]; readonly rejected: readonly GovernedSourceReference[]; readonly reason: string };
...
export function enrichGovernedClaims(input: ClaimEnrichmentEngineInput): ClaimEnrichmentEngineResult {
  ...
  const seedEvaluation = constructEnrichmentEvaluation(evaluationBody, seedIdentityDiscriminators);
  const enrichedClaims = drafts.map(draft => constructEnrichedClaim(draft.base.claimId, seedEvaluation.enrichmentEvaluationId, { claimType: draft.base.claimType, material: draft.base.material, status: draft.status, ownership: draft.ownership, sourceReferences: draft.sourceReferences, factualValues: draft.factualValues, sourceAvailable: draft.sourceAvailable, provenance: draft.provenance, observedAt: draft.observedAt, contentKind: draft.base.contentKind, boundedComplete: draft.boundedComplete, conflicts: draft.base.conflicts }));
  const claimOutcomes = seedRecords.map((record, index) => ({ ...record, enrichedClaimId: enrichedClaims[index].claimId }));
