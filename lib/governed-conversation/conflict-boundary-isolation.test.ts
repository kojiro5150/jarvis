...
describe("Sprint 3.92 pure-Node isolation and Option C proof", () => {
  ...
  it("finds no hidden production import", () => { const candidates = [...files("app"), ...files("lib")].filter(x => /\.(ts|tsx)$/.test(x) && !x.includes("conflict-boundary") && !x.includes("claims-conflicts-correction") && !x.includes("governed-publication-test-fixtures") && !x.includes("projection-composer.ts")); for (const path of candidates) for (const name of boundaryNames) expect(readFileSync(path, "utf8"), path).not.toContain(name); });
});
