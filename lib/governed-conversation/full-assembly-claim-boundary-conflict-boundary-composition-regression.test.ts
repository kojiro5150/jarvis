...
describe("Sprint 3.102 full-assembly composition regression", () => {
  ...
  it("preserves all six conflict outcomes instead of collapsing empty conflict arrays", async () => {
    ...
    const conflict = evaluateGovernedConversationalConflicts({ ruleset: CONFLICT_EVALUATION_RULESET, claimSet: direct.claimSet, observations: [observation("a", "a@example.com"), observation("b", "b@example.com")], requestedConflictClasses: ["source_value_contradiction"], referenceTime: FULL_ASSEMBLY_TIME, createdAt: FULL_ASSEMBLY_TIME, evaluationDiscriminator: "direct-conflict" });
...
describe("pure-Node Sprint 3.102 isolation", () => {
  it("preserves every protected byte and keeps the evaluator out of production", () => {
    ...
    for (const forbidden of ["app/api/chat", "context-builder", "useAgentConversation", "chat-execution"]) expect(evaluator).not.toContain(forbidden);
  });
});
