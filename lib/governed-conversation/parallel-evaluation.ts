...
export function evaluateConversationalScenario(scenario: ConversationalEvaluationScenario): ConversationalScenarioResult {
  ...
  const input = constructGovernedConversationalInput({ inputId: `evaluation:${scenario.scenarioId}`, threadId: `thread:${scenario.scenarioId}`, requestId: `request:${scenario.scenarioId}`, exchangeId: `exchange:${scenario.scenarioId}`, projectionId: "projection:synthetic-evaluation", projectionLineage: { threadId: `thread:${scenario.scenarioId}`, requestId: `request:${scenario.scenarioId}`, exchangeId: `exchange:${scenario.scenarioId}`, projectionId: "projection:synthetic-evaluation" }, referenceTime: REFERENCE_TIME.toISOString(), question: { text: scenario.question, timezone: "Etc/UTC", locale: "en-US" }, claims: rawClaims, sources: [{ sourceId: "synthetic:gmail", available: rawClaims.every(claim => claim.sourceAvailable), status: rawClaims.some(claim => claim.status === "unavailable") ? "unavailable" : "available", observedAt: REFERENCE_TIME.toISOString(), provenance: SYNTHETIC_EVALUATION_NOTICE }], compatibilityContext: scenario.messages.map(message => ({ contextId: `legacy:${message.id}`, legacySource: "OperationalState.gmailThreads", governedResourceId: message.id, classification: "compatibility", authority: "none", descriptiveFields: { subject: message.subject, from: message.from, snippet: message.snippet, unread: message.unread, sourceLabel: message.sourceLabel }, excludedHeuristicFields: ["important", "needsReply", "heuristicRanking", "requiringAttention"] })), conversationHistory: scenario.conversationHistory ?? [] });
