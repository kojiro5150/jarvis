...
function assertProjection(input: GovernedConversationalProjectionInput): void {
  ...
  if (conflictEvaluation) {
    if (conflictEvaluation.governedClaimSetId !== set.governedClaimSetId || conflictEvaluation.conflictEvaluationRulesetId === "" || conflictEvaluation.threadId !== input.threadId || conflictEvaluation.requestId !== input.requestId || conflictEvaluation.exchangeId !== input.exchangeId) throw new Error("conflict evaluation lineage mismatch");
