...
export function renderCalendarFreeTime(result: CalendarFreeTimeResult): string {
  if (result.status === "rejected") {
    ...
    if (result.reason === "calendar_refresh_required") return "Your Calendar connection has expired. Disconnect and reconnect Google Calendar, then try again.";
    if (result.reason === "calendar_not_connected") return "Google Calendar is not connected. Connect Google Calendar, then try again.";
