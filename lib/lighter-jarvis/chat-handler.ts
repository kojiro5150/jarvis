...
const PUBLIC_FRESHNESS_SIGNAL = new RegExp(
  [
    ...
    "\\bcurrent(?:ly)?\\s+(?:CEO|president|prime minister|leader|chair|chairperson|owner|version|release|price|policy|status|schedule|forecast|ranking|score|LTS|stable version)\\b",
    "\\bcurrent(?:ly)?\\s+(?:inflation|unemployment|employment|participation|interest|cash|exchange|mortgage|tax|growth|GDP|CPI)\\s+rate\\b",
    "\\bcurrent(?:ly)?\\s+rate\\b",
...
export function isMinimalFreshPublicFactQuestion(utterance: string): boolean {
  return isFreshnessSensitivePublicInformation(utterance)
    && !PUBLIC_EXPANSIVE_REQUEST.test(utterance);
}

export function enforceMinimalPublicFactReply(reply: string, utterance: string): string {
  if (!isMinimalFreshPublicFactQuestion(utterance)) return reply;
  ...
  return PUBLIC_SOURCE_SENTENCE.test(second)
    ? `${first} ${second}`
    : first;
}
