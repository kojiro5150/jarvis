...
export function proposeGmailSubjectList(newerThan: GmailSearchWindow): ProposedGmailSearchOperation {
...
export function evaluateGmailSearchAuthority(operation: ProposedGmailSearchOperation, currentUserUtterance: string): GmailSearchAuthorityDecision {
  // The explicit command grammar remains ID-only. Subject-list operations
  // require server-owned pending authorization so metadata release is explicit.
  const allowed = operation.resultMode === undefined
