...
const GMAIL_SEARCH_REQUEST = /^(?:please\s+)?(?:search|check|look\s+(?:in|through))\s+(?:my\s+)?(?:gmail|email|emails|inbox)\s+(?:for\s+)?(?:messages|emails|mail)?\s*(?:from|in|over)\s+(?:the\s+)?(?:last|past)\s+(day|24\s+hours?|week|7\s+days?)[?!.]?$/i;
