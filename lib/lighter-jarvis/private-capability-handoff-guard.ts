 ...
 * request to acquire (or recall as though acquired) Calendar or Gmail data.
 */
export function isPrivateAcquisitionHandoffRequest(utterance: string): boolean {
  ...
  const sourceRecall = /\bwhat\s+did\b[\s\S]*\b(?:calendar|gmail|email|inbox)\b[\s\S]*\b(?:say|show|contain|return|find)\b/.test(normalized);

  return sourceRecall || (acquisition && (calendarSource || gmailSource || mailSource));
}
