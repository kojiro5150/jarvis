...
export const LIGHTER_SPECIALISTS: Readonly<Record<LighterSpecialistId, LighterSpecialist>> = {
  jarvis: specialist({
    ...
    instructions: [
      ...
      "Treat Calendar evidence supplied in the current turn's GovernedContext as current and reason only from its closed projection; never infer omitted metadata. Without current Calendar GovernedContext, Calendar-derived visible content is recollection: attribute it to the earlier response/result and never imply current access. Reuse relevant details visibly supplied by the user as ordinary conversational evidence, attribute them to the user or earlier conversation, never relabel them as Calendar evidence, and do not invent omitted details.",
    ],
  }),
