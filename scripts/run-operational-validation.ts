  ...
  try {
    const result = await runAuthenticatedOperationalValidation({
      ...
      confirmChallenge: async (attestation) => {
        ...
        const answer = await terminal.question("Evidence challenge response: ");
        return { operator, answer };
      },
    });
    if (result.status === "AUTHENTICATED_VALIDATION_NOT_EXECUTED") {
      console.error("AUTHENTICATED_VALIDATION_NOT_EXECUTED: authentication was unavailable; no report or summary was produced.");
      process.exitCode = 2;
    } else {
      console.log(JSON.stringify({ status: result.status, reportPath: result.reportPath, operatorConfirmation: result.summary.operatorConfirmation, migrationRecommendation: result.summary.migrationRecommendation }, null, 2));
